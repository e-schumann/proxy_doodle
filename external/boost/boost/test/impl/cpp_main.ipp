...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_I_TRY {
        boost_part::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::boost_part::execution_monitor ex_mon;
        ...
        if( result == 0 )
            result = ::boost_part::exit_success;
        else if( result != ::boost_part::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::boost_part::exit_failure;
        }
    }
    BOOST_TEST_I_CATCH( ::boost_part::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::boost_part::exit_exception_failure;
    }
    BOOST_TEST_I_CATCH( ::boost_part::system_error, ex ) {
    ...
    if( result != ::boost_part::exit_success ) {
        ...
        ::boost_part::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
