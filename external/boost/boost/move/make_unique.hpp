...
{
   typedef ::boost_part::movelib::unique_ptr<T> t_is_not_array;
};

template<class T>
struct unique_ptr_if<T[]>
{
   typedef ::boost_part::movelib::unique_ptr<T[]> t_is_array_of_unknown_bound;
};
...
inline BOOST_MOVE_DOC1ST(unique_ptr<T>, 
   typename ::boost_part::move_upmu::unique_ptr_if<T>::t_is_not_array)
      make_unique(BOOST_FWD_REF(Args)... args)
{  return unique_ptr<T>(new T(::boost_part::forward<Args>(args)...));  }
...
{  return unique_ptr<T>(new (*boost_part::move_upmu::nothrow_holder<>::pnothrow)T(::boost_part::forward<Args>(args)...));  }
   ...
   #define BOOST_MOVE_MAKE_UNIQUE_CODE(N)\
      ...
      {  return unique_ptr<T>( new (*boost_part::move_upmu::nothrow_holder<>::pnothrow)T ( BOOST_MOVE_FWD##N ) );  }\
      //
...
{
    return unique_ptr<T>(new (*boost_part::move_upmu::nothrow_holder<>::pnothrow)T);
}
...
template<class T>
inline BOOST_MOVE_DOC1ST(unique_ptr<T>, 
   typename ::boost_part::move_upmu::unique_ptr_if<T>::t_is_array_of_unknown_bound)
      make_unique(std::size_t n)
{
    typedef typename ::boost_part::move_upmu::remove_extent<T>::type U;
...
{
    typedef typename ::boost_part::move_upmu::remove_extent<T>::type U;
    return unique_ptr<T>(new (*boost_part::move_upmu::nothrow_holder<>::pnothrow)U[n]());
}
...
inline BOOST_MOVE_DOC1ST(unspecified, 
   typename ::boost_part::move_upmu::unique_ptr_if<T>::t_is_array_of_known_bound)
