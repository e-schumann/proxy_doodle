...
struct deleter_types
{
   ...
   typedef ::boost_part::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
struct unique_ptr_data
{
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr_data(P p, BOOST_FWD_REF(U) d1) BOOST_NOEXCEPT
      : m_p(p), d(::boost_part::forward<U>(d1))
   {}
   ...
   : private D
{
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr_data(P p, BOOST_FWD_REF(U) d) BOOST_NOEXCEPT
      : D(::boost_part::forward<U>(d)), m_p(p)
   {}
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::boost_part::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
      , ::boost_part::rv<T>&
...
class unique_ptr
{
      ...
      : m_data(p)
   {
      ...
      BOOST_STATIC_ASSERT(( !::boost_part::move_upmu::missing_virtual_destructor
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type), BOOST_MOVE_SEEDOC(deleter_arg_type2) d2) BOOST_NOEXCEPT
      : m_data(pointer(), ::boost_part::move(d2))
   {}
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
      : m_data(u.release(), ::boost_part::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr( BOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> BOOST_RV_REF_END_IF_CXX11 u
      ...
      : m_data(u.release(), ::boost_part::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   unique_ptr& operator=(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
   {
      ...
      m_data.deleter() = ::boost_part::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   {
      this->reset(u.release());
      m_data.deleter() = ::boost_part::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   {
      ::boost_part::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::boost_part::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
