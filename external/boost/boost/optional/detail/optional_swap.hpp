...
# define BOOST_OPTIONAL_DETAIL_MOVE(EXPR_) boost_part::move(EXPR_)
#else
...
struct swap_selector<false>
{
    template <class T>
    static void optional_swap ( optional<T>& x, optional<T>& y ) 
    //BOOST_NOEXCEPT_IF(::boost_part::is_nothrow_move_constructible<T>::value && BOOST_NOEXCEPT_EXPR(boost_part::swap(*x, *y)))
    {
        if (x)
        {
            if (y)
...
template<class T>
struct optional_swap_should_use_default_constructor : boost_part::false_type {} ;
