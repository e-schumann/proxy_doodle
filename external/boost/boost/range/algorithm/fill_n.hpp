...
/// \pre n <= std::distance(boost_part::begin(rng), boost_part::end(rng))
template< class ForwardRange, class Size, class Value >
inline ForwardRange& fill_n(ForwardRange& rng, Size n, const Value& val)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    BOOST_ASSERT( static_cast<Size>(std::distance(boost_part::begin(rng), boost_part::end(rng))) >= n );
    std::fill_n(boost_part::begin(rng), n, val);
    return rng;
}
