...
{
   static char s_delim;
   static const int s_collate_type = ::boost_part::BOOST_REGEX_DETAIL_NS::find_sort_syntax(static_cast<c_regex_traits<char>*>(0), &s_delim);
   std::string result;
   //
...
bool BOOST_REGEX_CALL c_regex_traits<char>::isctype(char c, char_class_type mask)
{
   return
      ...
      || ((mask & char_class_blank) && (std::isspace)(static_cast<unsigned char>(c)) && !::boost_part::BOOST_REGEX_DETAIL_NS::is_separator(c))
      || ((mask & char_class_word) && (c == '_'))
      || ((mask & char_class_vertical) && (::boost_part::BOOST_REGEX_DETAIL_NS::is_separator(c) || (c == '\v')))
      || ((mask & char_class_horizontal) && (std::isspace)(static_cast<unsigned char>(c)) && !::boost_part::BOOST_REGEX_DETAIL_NS::is_separator(c) && (c != '\v'));
}
